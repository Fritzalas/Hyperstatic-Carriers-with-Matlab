function INT=simpson_get_gg(Fa,qa,g,La,Lb,Ma,X,icounter)
...
for i=1:1:len-1
  ...
  y=comp2(Fa,qa,g,La,Lb,Ma,C(1),icounter)+comp2(Fa,qa,g,La,Lb,Ma,C(N),icounter); %Me auto dinoume arxiki timi sto y prin mpei sto for loop pou tha exoume
  %Proto athroisma:
  a=(N-1)/2; %Ano orio athroismatos(den vazoume -1 logo tou systimatos anaforas pou exoume parei)
  for j=1:1:a
    y=y+4*comp2(Fa,qa,g,La,Lb,Ma,C(2*j),icounter); %Vazoyme epi tessera sta artia kai epi dyo sta perrita dioti allazei to systima anaoforas
  endfor
  %Deutero athroisma
  b=(N-1)/2; %Ano orio athroismatos
  %Ksekinaei apo to 2 dioti tin thesi 1 tin exoume etoimi idi
  for j=2:1:b
    y=y+2*comp2(Fa,qa,g,La,Lb,Ma,C((2*j)-1),icounter);
  endfor
